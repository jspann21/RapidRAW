...
export function useAppContextMenus(props: UseAppContextMenusProps) {
  ...
  const handleFolderTreeContextMenu = useCallback(
    (event: any, path: string | null, _isRootContext?: boolean, isRecent?: boolean) => {
      event.preventDefault();
      event.stopPropagation();
      ...
      const isCurrentlyPinned = pinnedFolders.includes(targetPath);
      const pinOption = isCurrentlyPinned
            ...
            onClick: () => {
              if (appSettings) {
                handleSettingsChange({
                  ...appSettings,
                  pinnedFolders: pinnedFolders.filter((folder: string) => folder !== targetPath),
                });
              }
            },
          }
        : {
            icon: Pin,
            label: t('contextMenus.folders.pin'),
            onClick: () => {
              if (appSettings) {
                handleSettingsChange({
                  ...appSettings,
                  pinnedFolders: [...pinnedFolders, targetPath],
                });
              }
            },
          };
